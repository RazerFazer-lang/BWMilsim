...
function audit(action,actor,detail){state.audit.unshift({id:id('a'),time:now(),action,actor:actor||'SYSTEM',detail});state.audit=state.audit.slice(0,100);}
function broadcast(event){
  const payload = `data: ${JSON.stringify({event,state})}\n\n`;
  for(const res of clients){try{res.write(payload);}catch{clients.delete(res);}}
}
function mutate(eventType,actor,detail,fn){fn();audit(eventType,actor,detail);saveState();broadcast({type:eventType,actor:actor||'SYSTEM',detail,at:now()});}
...
const server = http.createServer(async (req,res)=>{
  ...
  try{
    ...
    if(req.method==='GET'&&pathname==='/api/events'){
      ...
      res.write(`data: ${JSON.stringify({event:{type:'CONNECTED',at:now()},state})}\n\n`);
      clients.add(res);
      const timer=setInterval(()=>{try{res.write(': keepalive\n\n');}catch{clearInterval(timer);clients.delete(res);}},15000);
      req.on('close',()=>{clearInterval(timer);clients.delete(res);});
      return;
    }
    if(req.method==='POST'&&pathname==='/api/personnel'){
      const b=await parseBody(req);if(!b.name)return send(res,400,{error:'name required'});
      const p={id:id('p'),callsign:b.callsign||'UNASSIGNED',name:b.name,rank:b.rank||ranks.at(-1),role:b.role||'Schütze',unit:b.unit||'1. Gruppe',ready:false,active:true};
      mutate('PERSONNEL_UPDATED',b.actor||b.name,`Personal angelegt: ${b.name}`,()=>state.personnel.push(p));return send(res,201,p);
    }
    if(req.method==='PATCH'&&pathname.startsWith('/api/personnel/')){
      const pid=pathname.split('/').pop(),b=await parseBody(req),p=state.personnel.find(x=>x.id===pid);if(!p)return send(res,404,{error:'not found'});
      mutate('PERSONNEL_UPDATED',b.actor||p.name,`Personal geändert: ${p.name}`,()=>Object.assign(p,b.changes||{}));return send(res,200,p);
    }
    if(req.method==='POST'&&pathname==='/api/tasks'){
      const b=await parseBody(req);if(!b.title)return send(res,400,{error:'title required'});
    ...
    if(req.method==='PATCH'&&pathname.startsWith('/api/tasks/')){
      const tid=pathname.split('/').pop(),b=await parseBody(req),task=state.tasks.find(x=>x.id===tid);if(!task)return send(res,404,{error:'not found'});
    ...
    if(req.method==='POST'&&pathname==='/api/reports'){
      const b=await parseBody(req);if(!b.text)return send(res,400,{error:'text required'});
    ...
    if(req.method==='PATCH'&&pathname.startsWith('/api/markers/')){
      const mid=pathname.split('/').pop(),b=await parseBody(req),marker=state.markers.find(x=>x.id===mid);if(!marker)return send(res,404,{error:'not found'});
    ...
    if(req.method==='PATCH'&&pathname==='/api/operation'){
      const b=await parseBody(req);mutate('OPERATION_UPDATED',b.actor||'SYSTEM','Operationsstatus geändert',()=>Object.assign(state.meta,b.changes||{}));return send(res,200,state.meta);
    }
    if(req.method==='PATCH'&&pathname==='/api/aar'){
      const b=await parseBody(req);mutate('AAR_UPDATED',b.actor||'SYSTEM','AAR aktualisiert',()=>Object.assign(state.aar,b.changes||{}));return send(res,200,state.aar);
    }
